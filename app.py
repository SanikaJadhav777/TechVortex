...
if st.session_state.initial_story:
    ...
    with col_d2:
        if download_format == "Word (.docx)":
            file_buffer = build_word(st.session_state.initial_story)
            st.download_button(
                label="📄 Download Word",
                data=file_buffer,
            ...
            file_buffer = build_pdf(st.session_state.initial_story)
            st.download_button(
                label="📕 Download PDF",
                data=file_buffer,
